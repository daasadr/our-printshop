...
function debugCategoryLookup(
  ...
  console.log(
    `    Original type: ${typeof printfulId}, Numeric conversion: ${numericId}`
  );
  console.log(`    Map has key: ${categoriesMap.has(numericId)}`);
  console.log(`    Map size: ${categoriesMap.size}`);
  console.log(
    `    Map keys sample:`,
    Array.from(categoriesMap.keys()).slice(0, 10)
  );
  ...
  for (const [key, value] of categoriesMap) {
    if (key == printfulId) {
      // Use loose equality to check for type differences
      console.log(
        `    Found with loose equality! Key: ${key} (${typeof key}), printful_id: ${printfulId} (${typeof printfulId})`
      );
...
export async function checkCategoryExists(
  ...
  try {
    ...
    console.error(
      `Error checking category existence for printful_id ${printfulId}:`,
      error
    );
    return false;
  }
...
export async function getCategoryByPrintfulId(printfulId: number) {
  try {
    ...
    console.error(
      `Error fetching category by printful_id ${printfulId}:`,
      error
    );
    return null;
  }
  ...
  try {
    ...
    for (const [printfulId, categories] of groupedByPrintfulId) {
      if (categories.length > 1) {
        console.log(
          `Found ${categories.length} categories with printful_id ${printfulId}:`
        );
        ...
        for (const categoryToDelete of toDelete) {
          console.log(
            `  Deleting duplicate: ID ${categoryToDelete.id}, Name: ${categoryToDelete.name}`
          );
    ...
    console.log(
      `Cleanup completed: ${deletedCount} duplicate categories deleted`
    );
...
export async function syncCategories() {
  try {
    ...
    console.log(
      `Found ${existingCategories.length} existing categories in Directus with printful_id`
    );
    ...
    if (existingCategories.length > 0) {
      console.log(
        ...
        existingCategories.slice(0, 3).map((cat) => ({
          id: cat.id,
    ...
    console.log(
      `Created map with ${existingCategoriesMap.size} categories for lookup`
    );
    ...
    for (const printfulCategory of categories) {
      ...
      const normalizedParentId =
        printfulCategory.parent_id === 0 ? null : printfulCategory.parent_id;

      console.log(
        `Processing category "${printfulCategory.title}" (Printful ID: ${printfulCategory.id}) - ${exists ? "EXISTS" : "NEW"}`
      );
      if (!exists) {
        console.log(
          `  Raw parent_id: ${printfulCategory.parent_id}, normalized: ${normalizedParentId}`
        );
      ...
      if (exists) {
        ...
        const needsUpdate =
          ...
          normalize(existingCategory.parent_id) !==
            normalize(categoryData.parent_id) ||
          existingCategory.category_position !==
            categoryData.category_position ||
          normalize(existingCategory.size) !== normalize(categoryData.size) ||
          normalize(existingCategory.image_url) !==
            normalize(categoryData.image_url);

        if (needsUpdate) {
          console.log(`  UPDATE NEEDED for ${categoryData.name}:`);
          console.log(
            `    parent_id: ${normalize(existingCategory.parent_id)} -> ${normalize(categoryData.parent_id)}`
          );
          console.log(
            `    category_position: ${existingCategory.category_position} -> ${categoryData.category_position}`
          );
          console.log(
            `    size: ${normalize(existingCategory.size)} -> ${normalize(categoryData.size)}`
          );
          console.log(
            `    image_url: ${normalize(existingCategory.image_url)} -> ${normalize(categoryData.image_url)}`
          );

          await directus.request(
            updateItem("categories", existingCategory.id, categoryData)
          );
          console.log(
            `Updated category: ${categoryData.name} (ID: ${printfulCategory.id})`
          );
          updatedCount++;
        } else {
          console.log(
            `Skipped category (no changes): ${categoryData.name} (ID: ${printfulCategory.id})`
          );
          skippedCount++;
        }
      } else {
        // Double-check using direct database query to prevent duplicates
        const doubleCheckExists = await checkCategoryExists(
          printfulCategory.id
        );

        if (doubleCheckExists) {
          console.log(
            `  WARNING: Category ${categoryData.name} (ID: ${printfulCategory.id}) exists in DB but not in map! Skipping creation.`
          );
          skippedCount++;
        } else {
          // Create new category
          console.log(
            `  CREATING new category: ${categoryData.name} with data:`,
            {
          ...
          try {
            ...
            console.log(
              `Created category: ${categoryData.name} (ID: ${printfulCategory.id})`
            );
            createdCount++;
          } catch (error) {
            console.error(
    ...
    console.log(
      `Category sync completed: ${createdCount} created, ${updatedCount} updated, ${skippedCount} skipped`
    );
    ...
    throw new Error(
      `Failed to sync categories: ${JSON.stringify(error, null, 2)}`
    );
  }
}
